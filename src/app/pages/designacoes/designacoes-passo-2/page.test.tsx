...
describe("Designações - Passo 2 page", () => {
  ...
  it("renderiza accordions, habilita Próximo via setDisableProximo e submete com valores válidos", async () => {
    ...
    h.formDesignacaoData = {
      ...
      servidorIndicado: {
        ...
        cargo_base: "Cargo",
        funcao_atividade: "Func",
        cargo_sobreposto: "Sobreposto",
        cursos_titulos: "Cursos",
        dre: "DRE",
        unidade: "Unidade",
        codigo: "COD",
      },
    ...
    const { container } = render(<DesignacoesPasso2Page />);

    expect(screen.getByText("Dados do servidor indicado")).toBeInTheDocument();
    expect(screen.getByText("Portarias de designação")).toBeInTheDocument();
    expect(screen.getByTestId("resumo-designacao")).toBeInTheDocument();
    expect(screen.getByTestId("portaria-fields")).toBeInTheDocument();

    // props para ResumoDesignacao
    expect(h.resumoCalls[0]).toEqual(
    ...
    // Próximo começa desabilitado
    expect(screen.getByTestId("btn-proximo")).toBeDisabled();

    await user.click(screen.getByTestId("fill-valid-form"));
    expect(screen.getByTestId("btn-proximo")).not.toBeDisabled();

    // cobre callbacks inline do page.tsx
    await user.click(screen.getByTestId("resumo-editar"));
    await user.click(screen.getByTestId("btn-anterior-force"));
    await user.click(screen.getByTestId("btn-proximo"));
    expect(consoleSpy).toHaveBeenCalledWith("Proximo");
